...
class Tsid implements tsid.Tsid {
  ...
  static const int _tsidBytes = 8;
  static const int _tsidChars = 13;
...
class TsidFactory implements tsid.TsidFactory {
  ...
  static const int _nodeBits256 = 8;
  static const int _nodeBits1024 = 10;
  static const int _nodeBits4096 = 12;
  ...
  TsidFactory.fromBuilder(TsidFactoryBuilder builder) {
    _customEpoch = BigInt.from(builder.customEpoch);
    _nodeBits = BigInt.from(builder.nodeBits);
    ...
    _node = BigInt.from(builder.node) & _nodeMask;
  ...
  BigInt getTime() {
    var time = BigInt.from(_timeFunction());
  ...
  BigInt _getRandomCounter() {
    ...
    return BigInt.from(_random.nextInt()) & _counterMask;
  }
...
class TsidFactoryBuilder {
  static final int _tsidEpoch = DateTime.utc(2020).millisecondsSinceEpoch;

  int? _node;
  int _nodeBits = TsidFactory._nodeBits1024;
  bool _nodeBitsExplicit = false;
  int _customEpoch = _tsidEpoch;
  IRandom _random = ByteRandom.fromRandom(Random.secure());
  int Function() _timeFunction = () => DateTime.now().millisecondsSinceEpoch;
  ...
  TsidFactoryBuilder withDateTime(DateTime dateTime) {
    _timeFunction = () => dateTime.millisecondsSinceEpoch;
    return this;
  }
  ...
  int get nodeBits {
    if (!_nodeBitsExplicit) {
      ...
      if (configuredCount != null) {
        ...
        _nodeBits = (log(configuredCount) / log(2)).floor();
      }
    }
...
class Settings {
  ...
  static String? getProperty(String name) {
    final direct = String.fromEnvironment(name, defaultValue: '');
    if (direct.isNotEmpty) {
      return direct;
    }

    final upper = String.fromEnvironment(name.toUpperCase(), defaultValue: '');
    if (upper.isNotEmpty) {
      return upper;
    }
